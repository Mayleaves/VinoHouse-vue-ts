      ...
      <div class="tabList">
        <div class="table"
          ...
          <el-checkbox-group v-if="dishList.length > 0"
            ...
            <div v-for="(item, index) in filteredDishList"
...
export default class extends Vue {
  ...
  @Watch('seachKey')
  onSeachKeyChanged(newVal: string) {
    if (newVal && newVal.trim() !== '') {
      this.getDishForName(newVal)
    } else {
      if (this.dishType.length > 0) {
        //const curTypeId = this.dishType[this.keyInd]?.id  // 正确写法
        const curTypeId = this.dishType[this.keyInd] && this.dishType[this.keyInd].id
        if (curTypeId) this.getDishList(curTypeId)
      }
    }
  }

  // 新增计算属性
  get filteredDishList() {
    if (this.seachKey && this.seachKey.trim() !== '') {
      return this.allDishList.filter(function(item) {
        // item.dishName 和 item.name 可能为 undefined/null，先判空
        return (item.dishName && item.dishName.indexOf(this.seachKey) !== -1)
          || (item.name && item.name.indexOf(this.seachKey) !== -1);
      }.bind(this));
    }
  ...
  private getDishList(id: number) {
    queryDishList({ categoryId: id }).then(res => {
      if (res && res.data && res.data.code === 1) {
        ...
        if (!this.ids.has(id)) {
          this.allDishList = [...this.allDishList, ...newArr]  // 只会存储打开过分类下的酒水
        }
        this.ids.add(id)
      } else {
        this.$message.error(res.data.msg)
  ...
  private checkedListHandle(value: [string]) {
    ...
    // value 是一个只包含酒水名的数组 需要从 dishList中筛选出 对应的详情
    // 操作添加酒水
