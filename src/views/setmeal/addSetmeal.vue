<template>
  <div class="addBrand-container">
    ...
    <el-dialog v-if="dialogVisible"
      ...
      <!-- 搜索框：全局搜索需先查看全部酒水分类，否则仅能搜索浏览过分类下的酒水。
            todo 方法一：前端实现全量请求；方法二：后端实现查询功能，前端仅调用展示。 -->
      <el-input v-model="value"
                class="seachDish"
                placeholder="请输入酒水名称进行搜索"
                style="width: 293px; height: 40px"
                size="small"
                clearable
                @clear="onClear">  <!-- 监听清除按钮，触发返回到初始页面功能 -->
        <i slot="prefix"
      ...
      <AddDish v-if="dialogVisible"
               ...
               :seach-key="seachKey"
               :dish-list="dishList"
               @checkList="getCheckList" />  <!-- 弹窗的具体内容 -->
      <span slot="footer"
            class="dialog-footer">
    ...
    ;(this.$refs[formName] as any).validate((valid: any) => {
      if (valid) {
        if (this.dishTable.length === 0) {
          return this.$message.error('套餐下酒水不能为空')
        }
