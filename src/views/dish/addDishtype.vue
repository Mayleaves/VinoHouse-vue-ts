  ...
  <div :key="vueRest"
    ...
    <div :key="restKey"
      ...
      <el-form ref="ruleForm"
        ...
        <div class="address">
          <el-form-item label="酒水描述："
                        prop="region">
            <el-input v-model="ruleForm.description"
                      type="textarea"
                      :rows="3"
                      maxlength="200"
                      placeholder="酒水描述，最长200字" />
          </el-form-item>
        </div>
...
export default class extends Vue {
  ...
  get rules() {
    return {
      name: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            if (!value) {
              callback(new Error('请输入酒水名称'))
            } else {
              const reg = /^([A-Za-z0-9\u4e00-\u9fa5]){2,20}$/
              if (!reg.test(value)) {
                callback(new Error('酒水名称输入不符，请输入2-20个字符'))
              } else {
                callback()
              }
            }
      ...
      categoryId: [
        { required: true, message: '请选择酒水分类', trigger: 'change' }
      ],
      image: {
        required: true,
        message: '酒水图片不能为空'
  ...
  private getFlavorListHand() {
    ...
    this.dishFlavorsData = [
      { name: '规格', value: ['375ml', '700ml', '1L', '1.75L'] },
      { name: '酒精度', value: ['低度(<10%)', '中度(10%-20%)', '高度(>20%)'] },
      { name: '配料添加', value: ['糖浆', '苦精', '香草荚', '豆蔻', '海盐', '烟熏风味'] },
      { name: '适饮温度', value: ['冰镇(4-8℃)', '冷藏(8-12℃)', '常温(12-18℃)', '温热(18-25℃)'] }
    ]
  }
  ...
  private submitForm(formName: any, st: any) {
    ;(this.$refs[formName] as any).validate((valid: any) => {
      ...
      if (valid) {
        if (!this.ruleForm.image) return this.$message.error('酒水图片不能为空')
